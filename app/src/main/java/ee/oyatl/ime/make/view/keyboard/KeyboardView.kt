...
abstract class KeyboardView(
    ...
    protected val longPressDuration: Long = preferences.getFloat("behaviour_long_press_duration", 250f).toLong()
    protected val longPressAction: String = preferences.getString("behaviour_long_press_action", null) ?: "shift"
    protected val repeatInterval: Long = preferences.getFloat("behaviour_repeat_interval", 50f).toLong()

    protected val slideAction = preferences.getString("behaviour_slide_action", "seek")
    ...
    protected fun onTouchDown(key: KeyWrapper, pointerId: Int, x: Int, y: Int) {
        ...
        handler?.postDelayed({
            if(key.key.repeatable || longPressAction == "repeat") {
                repeater()
